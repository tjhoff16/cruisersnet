		<?php $label_terms = wp_get_post_terms($post->ID,'cnet_marina_settings',array('fields'=>'names'));?>
		<?php if ( empty($label_terms) || $label_terms[0]!='Not A Marina (Fuel Only)' ) : ?>
			<?php $bg = (get_field('background_color') ? 'post-bg-'.get_field('background_color') : ''); ?>
			<li <?php post_class($bg); ?>>
				<?php //print_r($label_terms); ?>
                ...
                <?php
                /*  if this doesn't not have any settings, begin showing all information. (settings currently include "label" terms and "not-a-marina" term  */
        	...
        	<div class="marina-info">
        		...
        		<table class="info-table" style="<?php if (get_post_meta($post->ID,'marina_sponsor_graphic',true)) { echo 'width: 80%;'; } else { echo 'width: 100%'; } ?>">
               	...
               	<?php
               	/* the label terms are not empty, meaning we want to display differenty */ 
               	} else {   	
               	
               		if (get_field('label_disclaimer') == 'enable') : the_field('disclaimer_text'); endif;
